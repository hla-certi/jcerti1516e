...
import certi.rti.impl.CertiReceivedInteraction;
import certi.rti.impl.CertiReflectedAttributes;
...
/**
 * This class is used to process all the primary data types and to store them
 * into a Vector<Byte>. It can handle sending and receiving of
 * <code>CertiMessage</code> from streams.
 *
 * The JVM is big endian, so all the write methods are big endians. The read
 * methods can handle both big and little endians.
 *
...
public class MessageBuffer {
	...
	/**
	 * This method set the buffer back in the state just after its creation should
	 * be used before re-using the buffer to write. It is already automatically done
	 * when a message is read from the socket.
	 */
	public void reset() {
		buffer.clear();
	}
	...
	/**
	 * Write byte array to buffer. First is the length of the array, stored as a
	 * integer, then the bytes themselves.
	 *
	...
	/**
	 * Read an array of bytes from the buffer. First read the size (integer) and
	 * then the vales.
	 *
	 * @return read array
	 */
	 ...
	 * 
	 * Beware! there are no unsigned integers of any size in java. This methods
	 * assume that the short it is trying to read is inside the bounds of a signed
	 * short. There are no conversion to an int if it is not the case.
	 *
	 * @return read short
	 * @throws NoSuchElementException
	 */
	 ...
	 * 
	 * Beware! there are no unsigned integers of any size in java. This methods
	 * assume that the int it is trying to read is inside the bounds of a signed
	 * int. There are no conversion to an long if it is not the case.
	 *
	 * @return an int
	 * @throws NoSuchElementException
	 */
	...
	/**
	 * The endianness is taken care of. Beware! there are no unsigned integers of
	 * any size in java. This methods assume that the long it is trying to read is
	 * inside the bounds of a signed long. There are no conversion to something
	 * bigger if it is not the case.
	 * 
	 * @return
	 * @throws NoSuchElementException
	 */
	...
	public void write(float f) throws NoSuchElementException {
		// The floatToIntBits return exactly what we are looking for: IEEE 754 coding of
		// the double
	...
	/**
	 * Write a String: first an int with the size, then all the char, each one coded
	 * on two bytes in UTF-16BE.
	 * 
	...
	synchronized public void send() throws IOException {
		...
		Vector<Byte> message = new Vector<>();
	...
	public void write(hla.rti.AttributeHandleSet attributes) {
		...
		for (int i = 1; i < attributes.size(); i++) { // Starting at 1 since first value was written with
														// iterator.first()
	...
	public List<CertiExtent> readExtents() {
		...
		List<CertiExtent> extents = new ArrayList<>(numberOfExtents);
	...
	public CertiHandleValuePairCollection readHandleValuePairCollection() {
		...
		List<Integer> handles = new ArrayList<>(size);
	...
	public List<Long> readRegions() {
		int n = this.readInt();
		List<Long> regions = new ArrayList<>(n);
