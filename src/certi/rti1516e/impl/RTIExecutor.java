...
public class RTIExecutor {
	...
	public static boolean checkLocalHost() {
		...
		if (certiHost != null && !certiHost.equals("localhost") && !certiHost.equals("127.0.0.1")) {
			System.out.println("WARNING : The environment variable CERTI_HOST, which has value: " + certiHost
					+ ", is neither localhost nor 127.0.0.1. We cannot launch an RTI at that address. ");
...
//            + certiHost + ", is neither localhost nor 127.0.0.1. We cannot launch an RTI at that address. ");
	...
	public void executeRTIG() throws RTIinternalError {
		...
		if (isRunning()) {
			System.out.println(
					"WARNING : Expected to launch a new RTIG, but one is running already. We will use this one. To launch a new RTIG with the test, kill the process before launch the test.");
		} else {
