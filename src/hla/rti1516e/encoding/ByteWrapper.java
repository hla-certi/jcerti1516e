...
public class ByteWrapper {
	...
	/**
	 * Constructs a <code>ByteWrapper</code> backed by the specified byte array.
	 * (Changes to the ByteWrapper will write through to the specified byte array.)
	 *
	 ...
	 *
	 * @throws ArrayIndexOutOfBoundsException if <code>length</code> bytes can not
	 *                                        be read
	 */
	...
	public final int getInt() {
		...
		int pos = _pos;
		byte[] buffer = _buffer;
		int value = ((buffer[pos] & 0xFF) << 24) + ((buffer[pos + 1] & 0xFF) << 16) + ((buffer[pos + 2] & 0xFF) << 8)
				+ (buffer[pos + 3] & 0xFF);
		_pos += 4;
	...
	/**
	 * Reads the next byte of the ByteWrapper. The current position is increased by
	 * 1.
	 *
	 * @return decoded value
	 *
	 ...
	 * <code>dest</code>. The current position is increased by
	...
	/**
	 * Writes <code>value</code> to the ByteWrapper as a hi-endian 32-bit integer.
	 * The current position is increased by 4.
	 *
	...
	/**
	 * Writes a byte array to the ByteWrapper and advances the current posisiton by
	...
	/**
	 * Creates a <code>ByteWrapper</code> backed by the same byte array using the
	 * current position as its offset.
	 *
	 * @return a new <code>ByteWrapper</code> backed by the same byte array starting
	...
	/**
	 * Creates a <code>ByteWrapper</code> backed by the same byte array using the
	 * current position as its offset, and the specified <code>length</code> to mark
	 * the limit.
	 *
	 * @param length length of the new <code>ByteWrapper</code>
	 *
	 * @return a new <code>ByteWrapper</code> backed by the same byte array starting
	 *         at the current position with the defined <code>length</code>
	 *
