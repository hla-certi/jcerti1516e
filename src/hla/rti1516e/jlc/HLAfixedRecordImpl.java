...
/**
 * HLAfixedRecordImpl is the implementation of type HLAfixedRecord It is an
 * array with a variable size, wich contains elements witch can be all types
 * (simple or complexe) The element doesn't have to be all the same tyoe
 */
public class HLAfixedRecordImpl extends DataElementBase implements hla.rti1516e.encoding.HLAfixedRecord {
	...
	/**
	 * Calcul the octet boundary HLAfixedRecord octet boundary is the max of all the
	...
	/**
	 * Encode the HLAfixedRecord Put in the byteWrapper : - each value (expect last
	...
	/**
	 * Calcul the length necessary to encode this HLAfixedRecord The lenght is the
	 * sum of : - the size of each value (expect last one) + padding - the size of
	 * the last value without padding
	 * 
	...
	public int getEncodedLength() {
		...
		for (DataElement e : this.values) {
			elength += e.getEncodedLength()
					+ calculPaddingAfterEachElements(e.getEncodedLength(), this.getOctetBoundary());
		}
		return elength;
	}

	/**
	 * Decode the HLAfixedRecord The structure must have been defined before the
	 * decoding
	 * 
