...
/**
 * HLAvariableArrayImpl is the implementation of type HLAvariableArray It is an
 * array with a variable size, wich contains elements with the same type T
 * 
...
public class HLAvariableArrayImpl<T extends DataElement> extends DataElementBase
	...
	/**
	 * Calcul the octet boundary HLAVariableArray octet boundary is the max of all
	 * the values's octet boundary If the array is empty (during decoding), octet
	 * boundray is calculated with the size of an object T
	 * 
	...
	/**
	 * Encode the HLAVariableArray Put in the byteWrapper : - the size of the array
	 * - each value (expect last one) with padding - last value without padding
	 * 
	...
	/**
	 * Calcul the length necessary to encode this HLAVariableArray The lenght is the
	 * sum of : - the size lenght aligned (8) - the size of each value (expect last
	 * one) with padding - the size of the last value without padding
	 * 
	...
	public int getEncodedLength() {
		int elength = 8;
		// int padding =
		// calculPaddingAfterEachElement(this.values.get(0).getEncodedLength(),
	...
	/**
	 * Decode the HLAVariableArray 2 cases possible : - if the objets can be created
	 * with a factory and the factory is initialized, we create new element with the
	 * factory and add them in the array - if the factory is not initialized (for
	 * exemple for complexe types with differents structures), then the structure
	 * must have been defined before the decoding If we are not in one of the cases,
	...
	public void decode(ByteWrapper byteWrapper) throws DecoderException {
		...
		if (this.efactory != null) {
			...
			for (int i = 0; i < nbElem - 1; ++i) {
				...
				// Pass the padding
				byteWrapper
						.advance(this.calculPaddingAfterEachElement(elem.getEncodedLength(), this.getOctetBoundary()));
			}
			...
			for (int i = 0; i < nbElem - 1; i++) {
				T elem = values.get(i);
				elem.encode(byteWrapper);
				int paddingSize = calculPaddingAfterEachElement(values.get(i).getEncodedLength(),
						this.getOctetBoundary());
				if (paddingSize > 0) {
					byte[] paddingBytes = new byte[paddingSize];
					for (int j = 0; j < paddingSize; j++)
