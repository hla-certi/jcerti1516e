...
/**
 * HLAvariantRecordImpl is the implementation of type HLAvariantRecord It is a
 * map witch associate a discriminant with the alternative All the discriminant
 * have the same type T, witch can be a simpple or a complexe type The
 * alternatives can have each a different type, simple or complexe
 * 
...
public class HLAvariantRecordImpl<T extends DataElement> extends DataElementBase
	...
	/**
	 * Constructor of HLAvariantRecordImpl with a discriminant T Change the value of
	 * current discriminent but does'nt add anything to the map
	 * 
	 * @param discriminant : Discriminant of type T. All the other discriminant will
	...
	/**
	 * Constructor of HLAvariantRecordImpl with a discriminant T Change the value of
	 * current discriminent and add the couple (discriminant, alternative) to the
	 * map
	 * 
	 * @param discriminant : Discriminant of type T. All the other discriminant will
	 *                     have to be of the same type.
	...
	/**
	 * Calcul the octet boundary HLAVariantRecord octet boundary is the max of
	 * discriminant's octet boundary and each alternative
	 *
	...
	/**
	 * Calcul the length necessary to encode this HLAVariantRecord The lenght is the
	 * sum of the discriminent lenght, its alternative and the padding
	 * HLAVariantRecord octet boundary is the max of discriminant's octet boundary
	...
	public int getEncodedLength() {
		...
		if (this.getValue() != null) {
			...
			elength += this.calculPaddingAfterDiscriminant(this.discriminant.getEncodedLength(),
	...
	public void encode(ByteWrapper byteWrapper) throws EncoderException {
		if (this.discriminant == null)
			throw new EncoderException("HLAvariantRecord discriminent is null");
		if (!this.variants.containsKey(discriminant))
			throw new EncoderException("HLAvariantRecord discriminent doesn't have value affected");
	...
	/**
	 * Decode the VariantRecord The structure of the variant must be defined before
	 * decoding it
	 * 
	...
	/**
	 * Add a new pair of a discriminent and this alternative in the variant Also
	 * change the value of the current discriminent
	 * 
	 * @param Discriminant
	...
	public DataElement getValue() {
		if (this.discriminant == null)
			return null;
		if (!variants.containsKey(this.discriminant))
			return null;
