...
/**
 * Implementation of HLAinteger64Time The time value is represented on a
 * HLAinteger64BE value
 */
public class HLAinteger64TimeImpl implements HLAinteger64Time {
	...
	/**
	 * Create a new HLAinteger64Time whitch is the sum of the HLAinteger64Time and
	 * the one in parameter
	 * 
	 * @param val : HLAinteger64Time to add
	 * @return : a new HLAinteger64Time whitch is the sum of the HLAinteger64Time
	...
	/**
	 * Create a new HLAinteger64Time whitch is the difference of the
	 * HLAinteger64Time and the one in parameter
	 * 
	 * @param val : HLAinteger64Time to add
	 * @return : a new HLAinteger64Time whitch is the difference of the
	 *         HLAinteger64Time and the one in parameter
	 */
	@Override
	...
	public HLAinteger64Interval distance(HLAinteger64Time val) {
		HLAinteger64Interval integer64Interval = new HLAinteger64IntervalImpl(
				Math.abs(this.getValue() - val.getValue()));
		return integer64Interval;
	}

	/**
	 * Compare the value of this HLAinteger64Time the value of the HLAinteger64Time
	 * other
	 * 
	...
	public void encode(byte[] buffer, int offset) throws CouldNotEncode {
		...
		/*
		 * if(buffer.length - offset < this.encodedLength()) throw new
		 * CouldNotEncode("Length disponible in the buffer is not sufficient"); long
		 * value = this.time.getValue(); buffer[offset] = (byte)(value >>> 56);
		 * buffer[offset+1] = (byte)(value >>> 48); buffer[offset+2] = (byte)(value >>>
		 * 40); buffer[offset+3] = (byte)(value >>> 32); buffer[offset+4] = (byte)(value
		 * >>> 24); buffer[offset+5] = (byte)(value >>> 16); buffer[offset+6] =
		 * (byte)(value >>> 8); buffer[offset+7] = (byte)(value >>> 0);
		 */
	}
