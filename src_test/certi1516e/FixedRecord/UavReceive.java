			...
			try {
				...
				for (AttributeHandle attributeHandle : attributeHandleSet) {
					if (attributeHandle.hashCode() == arrayAttributeHandle.hashCode()) {
						...
						HLAfloat32BE float32BE = new BasicHLAfloat32BEImpl();
						HLAinteger32BE integer32BE = new BasicHLAinteger32BEImpl();
						HLAinteger64LE integer64LE = new BasicHLAinteger64LEImpl();
						...
						fixedRecord.decode(bw);
						LOGGER.info("     --> Receive FixedRecord with values : {" + ((HLAfloat32BE)fixedRecord.get(0)).getValue() + ", "
