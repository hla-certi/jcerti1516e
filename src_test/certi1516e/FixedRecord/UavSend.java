package certi1516e.FixedRecord;
...
public class UavSend {
	...
	public void runFederate(double timeStepArg, double uptdateTimeArg, double lookaheadArg) throws Exception {
		...
		LOGGER.info("     6 Uav Send");
			...
			HLAfloat32BE float32BE = new BasicHLAfloat32BEImpl((float) 1.5);
			HLAinteger32BE integer32BE = new BasicHLAinteger32BEImpl(2);
			HLAinteger64LE integer64LE = new BasicHLAinteger64LEImpl(10);

			// Declaration of the array
			HLAfixedRecord fixedRecord = new HLAfixedRecordImpl();
			// We add our 3 elements in the array
			fixedRecord.add(float32BE);
			fixedRecord.add(integer32BE);
			fixedRecord.add(integer64LE);

			// We create a byte[] to get and send encode result
			// Function getEncodedLength return array encoded length
			byte[] arrayAttribute = new byte[fixedRecord.getEncodedLength()];
			...
			// We encode the array
			fixedRecord.encode(byteWrapper);
			...
			try {
				...
				LOGGER.info("     --> Send FixedRecord with values : {" + ((HLAfloat32BE)fixedRecord.get(0)).getValue() + ", "
						+ ((HLAinteger32BE)fixedRecord.get(1)).getValue() + ", "
						+ ((HLAinteger64LE)fixedRecord.get(2)).getValue() + "}");
			} catch (Exception e) {
