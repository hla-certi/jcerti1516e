 ...
 * 1516-2010 Extended (HLA 1516e), and introduces some parameters. If there
 * is no RTIG running, it launches the RTIG (if launched first), otherwise
 * it uses the existent one (on the machine specified by $CERTI_HOST. 
 * It creates a federation called federationExecutionName
 * (if launched first), then joins this federation, 
 ...
 * ant -DtimeStep=20 -DupdateTime=5 -Dlookahead=1 UAVSend1516eString-run
...
public class FlexUavReceiveString {
    
    private final static Logger LOGGER = Logger.getLogger(FlexUavReceiveString.class.getName());
    // private static final double stopTime = 50; //If used in "RAV loop"
    
    // If it is the first launched federate it launches the RTIG, otherwise it connects to
            ...
            FederateOwnsAttributes
    {
        ...
        try {
            ...
            if(!(RTIExecutor.checkLocalHost()))
                throw new ConnectionFailed("Connection to the RTIG failed. You are trying to connect to a RTIG on a remote machine, but no RTIG was found.");
            else
        ...
        //  rtia_receive.joinFederationExecution(federateName, federateType, federationExecutionName);
