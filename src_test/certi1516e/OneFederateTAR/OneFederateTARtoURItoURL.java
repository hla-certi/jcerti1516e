 ...
 * creates and joins a federation called TimeAdvanceTAR. 
 * </p>
 * <p>
 * It creates the federation using the deprecated service:
 * createFederationExecution(federationExecutionName, fom.toURI().toURL())
 * This way only works if the federate is in the same computer as the RTIG. 
 * FIXME: The fom file must be in jcerti folder, even if it is in $CERTI_FOM_PATH
 * </p>
 * <p>
 * The synchronization point 'InitSync' is registered by this federate. It
 * sends synchronizationPointAchieved() and the RTI provides immediately the 
...
public class OneFederateTARtoURItoURL {

	private final static Logger LOGGER = Logger.getLogger(OneFederateTARtoURItoURL.class.getName());
	...
	private static double BLOCKING_TIME = 0.1; // may be 1.0 is a better vaule.
	...
	public static void main(String[] args) throws Exception {
		try {
			...
			new OneFederateTARtoURItoURL().runFederate(timeStepArg, updateTimeArg, lookahead);
		} catch (NumberFormatException | ArrayIndexOutOfBoundsException exception) {
			// Default values
			new OneFederateTARtoURItoURL().runFederate(10.0, 0.2, 0.1);
		}
	}
