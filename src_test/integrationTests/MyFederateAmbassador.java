...
public class MyFederateAmbassador extends NullFederateAmbassador {
	...
	public float fomAttributeValue;
	private final static Logger LOGGER = Logger.getLogger(MyFederateAmbassador.class.getName());
	 ...
	 */
	public void initialize(RTIambassador rtia) throws NameNotFound, FederateNotExecutionMember, RTIinternalError,
			ObjectClassNotDefined, AttributeNotDefined, OwnershipAcquisitionPending, SaveInProgress, RestoreInProgress,
		...
		lookahead = new CertiLogicalTimeInterval1516E(0.1);
		timeStep = new CertiLogicalTime1516E(1.0);
		timeAdvance = new CertiLogicalTime1516E(1.0);
		...
		try {
			...
			for (AttributeHandle attributeHandle : attributeHandleSet) {
				if (attributeHandle.hashCode() == textAttributeHandle.hashCode()) {
					...
					value.decode(bw);
					textAttributeValue = value.getValue();
				...
				if (attributeHandle.hashCode() == fomAttributeHandle.hashCode()) {
					...
					ByteWrapper bw = theAttributes.getValueReference(attributeHandle);
					value.decode(bw);
					fomAttributeValue = value.getValue();
	...
	public void timeAdvanceGrant(LogicalTime theTime) throws FederateInternalError {
		...
		updateTime = new CertiLogicalTime1516E(((CertiLogicalTime1516E) localHlaTime).getTime() + 0.2);
