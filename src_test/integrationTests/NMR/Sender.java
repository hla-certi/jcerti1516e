...
public class Sender {
    ...
    public boolean send(float expectedFloat, String expectedString, LogicalTime updateTime) throws IOException, InterruptedException {
        ...
        try {
            ...
            rtia_send.createFederationExecution(federationExecutionName,  fomName);
...
//        while (i-- > 0) {
        ...
        SendandReceiveValues_NMR.LOGGER.info("Sender ------- 6.1 UAV with time = " + ((CertiLogicalTime1516E)mya_send.updateTime).getTime());
        try {
            rtia_send.updateAttributeValues(mya_send.myObject, attr, tagBuffer, mya_send.updateTime);
        } catch (Exception e){
            SendandReceiveValues_NMR.LOGGER.info("Sender ------- Timestamp must be bigger than (localHlaTime + lookahead)" );
            return false;
        }
        // The federate ask to advance to (current logical time + timeStep)
        SendandReceiveValues_NMR.LOGGER.info("Sender ------- 6.2 NMR with time = " + ((CertiLogicalTime1516E)mya_send.timeAdvance).getTime());
        try {
        ...
        try
        {
            while(federationIsActive){
                try{
                    ...
                    SendandReceiveValues_NMR.LOGGER.warning("Sender ------- Federation execution doesn't exist.  May be the Federation was destroyed by some other federate." );
                    federationIsActive = false;
