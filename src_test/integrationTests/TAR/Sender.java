...
public class Sender {
    ...
    //Set times values
    private double updateTimeTAR; //Set with parameter expectedTime, to check if the time is correct at test end (in Receiver)
    ...
    public boolean send(float expectedFloat, String expectedString, LogicalTime updateTime) throws IOException, InterruptedException {
        ...
        try {
            ...
            if(!RTIExecutor.checkLocalHost())
                SendandReceiveValues_TAR.LOGGER.severe("Connection to the RTIG failed. You are trying to connect to a RTIG of an other machine, but no RTIG was found.");
            else
                SendandReceiveValues_TAR.LOGGER.severe("Connection to the RTIG failed. There is probably no RTIG running.");
            return false;
        }
        ...
        SendandReceiveValues_TAR.LOGGER.info("Sender ------- 6.1 UAV with time = " + ((CertiLogicalTime1516E)mya_send.updateTime).getTime());
        try {
            rtia_send.updateAttributeValues(mya_send.myObject, attr, tagBuffer, mya_send.updateTime);
        } catch (Exception e){
            SendandReceiveValues_TAR.LOGGER.info("Sender ------- Timestamp must be bigger than (localHlaTime + lookahead)" );
            return false;
        }
        // The federate ask to advance to (current logical time + timeStep)
        SendandReceiveValues_TAR.LOGGER.info("Sender ------- 6.2 TAR with time = " + ((CertiLogicalTime1516E)mya_send.timeAdvance).getTime());
        try {
        ...
        try
        {
            while(federationIsActive){
                try{
                    ...
                    SendandReceiveValues_TAR.LOGGER.warning("Sender ------- Federation execution doesn't exist.  May be the Federation was destroyed by some other federate." );
                    federationIsActive = false;
