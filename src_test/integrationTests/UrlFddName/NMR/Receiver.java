...
public class Receiver {
	...
	public boolean receive(float expectedFloat, String expectedString, LogicalTime expectedTime)
		...
		try {
			File fdd = new File(fddName);
			rtia_receive.createFederationExecution(federationExecutionName, fdd.toURI().toURL());
			flagCreator = true;
		...
		try {
			URL[] joinModules = { (new File("uav.xml").toURI().toURL())};
		...
		while (((CertiLogicalTime1516E) mya_receive.localHlaTime).getTime() <= ((CertiLogicalTime1516E) expectedTime)
			...
			try {
				...
				SendandReceiveValues_NMR.LOGGER.info("Receiver .........6.1 NMR with time="
						+ ((CertiLogicalTime1516E) mya_receive.timeAdvance).getTime());
		...
		try {
			while (federationIsActive) {
				try {
					...
					SendandReceiveValues_NMR.LOGGER.info(
